...
if __name__ == "__main__":
    ...
    # Loading the older processed data
    y_num = "3"
    file_name = f"Y{y_num}"
    ...
    min_cbar_value_array = [12, -3, -1, 12, -70, 0]
    max_cbar_value_array = [18, 3, 1, 18, 70, 70]
    min_mask_value_array = [12, -3, -1, 12, -70, 0]
    max_mask_value_array = [18, 3, 1, 18, 70, 70]
    ...
    with PdfPages(pdf_path) as pdf:
        ...
        for i, var in enumerate(variables_to_plot):
            ...
            color_data = griddata(
                np.array([x_meshgrid_global.flatten(), y_meshgrid_global.flatten()]).T,
                np.array(loaded_data[var].values),
                (x_meshgrid_global, y_meshgrid_global),
                method="linear",
            )

            # Check the range of the data and adjust masking if necessary
            if np.all(np.isnan(color_data)):
                print(f"Warning: Data for {var} is all NaN. Skipping plot.")
                continue

            # Mask the data to the color ranges
            color_data = np.ma.masked_where(
                color_data > max_mask_value_array[i], color_data
            )
            color_data = np.ma.masked_where(
                color_data < min_mask_value_array[i], color_data
            )

            # Plotting on the correct axis
            subsample_color = 2
            x_mesh_sub = x_meshgrid_global[::subsample_color, ::subsample_color]
            y_mesh_sub = y_meshgrid_global[::subsample_color, ::subsample_color]
            color_data_sub = color_data[::subsample_color, ::subsample_color]
            ...
            cax = ax.contourf(
                ...
                levels=40,
                extend="both",
                vmin=min_cbar_value_array[i],
                vmax=max_cbar_value_array[i],
            )

            mid_cbar_value = np.mean([min_cbar_value_array[i], max_cbar_value_array[i]])

            # Create a colorbar for each subplot
            cbar = fig.colorbar(
                cax,
                ax=ax,  # Specify the axis for the colorbar
                ticks=[
                    min_cbar_value_array[i],
                    mid_cbar_value,
                    max_cbar_value_array[i],
                ],
                orientation="horizontal",
                format=mticker.FixedFormatter(
                    [
                        f"< {min_cbar_value_array[i]}",
                        f"{mid_cbar_value}",
                        f"> {max_cbar_value_array[i]}",
                    ]
                ),
                extend="both",
            )
            ...
            # labels[0].set_verticalalignment("top")
            # labels[-1].set_verticalalignment("bottom")
            cbar.set_label(f"{var}", rotation=0)
            ax.set_title(var)
